...
import { Producto, Carrito, Mensaje } from '../interfaces';
...
class Files {
  ...
  async saveItem(item: Producto | Carrito | Mensaje) {
  ...
  async getById(id: String) {
    try {
      ...
      let item = info.find((e: Producto | Carrito | Mensaje) => e.id == id);
  ...
  async deleteById(id: String) {
    ...
    try {
      ...
      const index = info.findIndex(
        (item: Producto | Carrito | Mensaje) => item.id == id
      );
  ...
  async update(item: Producto | Carrito | Mensaje) {
    const filePath = path.resolve(__dirname, this.archivo);
    try {
      // Intento leer el archivo
      ...
      const index = info.findIndex(
        (elem: Producto | Carrito | Mensaje) => item.id === elem.id
      );
...
// Usaremos el archivo 'products.json' ubicado en la raíz del proyecto para guardar los productos
const messagesObj = new Files('../../chatMessages.json');

export { productsObj, cartsObj, messagesObj };
