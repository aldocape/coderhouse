...
productForm.addEventListener('submit', (event) => {
  ...
  // Hago un 'POST' al endpoint de graphql y le paso la mutation con el método save y el producto nuevo
  requestHandler('/graphql', 'POST', mutation).then((newProd) => {
    ...
    if (newProd.data.saveProduct.id) {
      divProd.innerHTML = '';

      // Emite un evento 'newProduct' al server
      socket.emit('newProduct', newProd.data.saveProduct);
    }
    ...
    divProd.innerHTML = 'Producto nuevo creado con éxito';
  });
});
...
// Función fetch para traer los productos almacenados en la DB cuando se carga el sitio
requestHandler('/graphql', 'POST', query).then((products) => {
  if (products.data.getAllProducts && products.data.getAllProducts.length) {
    const allProds = products.data.getAllProducts;
